...
def process_claims(df, invoiceprefix):
    ...
    for current_mrn in All_MRNs:
        ...
        for episode_range in episode_ranges:
            ...
            claims_dict = {'Description of Service': description_of_service,
            ...
            new_claims_df = pd.concat(claims_dict,
                                      axis=1)
            ...
            # Apply Formatting
            new_claims_df['Item'] = new_claims_df['Item'].apply(
                dataformatter.formatter_number0)
            new_claims_df['Billed to Insurance'] = new_claims_df['Billed to Insurance'].apply(dataformatter.formatter_currency_with_cents)
            new_claims_df['Insurance Adjustement'] = new_claims_df['Insurance Adjustement'].apply(
                dataformatter.formatter_currency_with_cents)
            new_claims_df['Payment to Agency'] = new_claims_df['Payment to Agency'].apply(
                dataformatter.formatter_currency_with_cents)
            new_claims_df['Patient\'s Responsible Amount '] = new_claims_df['Patient\'s Responsible Amount '].apply(
                dataformatter.formatter_currency_with_cents)
            ...
            create_patient_responsibity_invoice("file_"+current_mrn+episode_range+'.pdf', invoicenum = invoicenum, claims_df= new_claims_df, to_address_parts=full_address, insurance_info=insurance_info)
